...
# Install Git aliases for conv# Utility to set an alias
set_alias() {
  name="$1"
  value="$2"

  if [ "$FO# Shortcut: git amend => commit --amend --no-edit
...
echo '  git amend   # to amend without changing the message' ]; then
...
# Create a "git commit-<tag>" alias => [TAG] : message Style: [TYPE] : message  (ex: [FIX] : corrected NPE on upload)
...
Alias générique:
  git cm <type> "message"
  Types supportés: fix, feat, docs, refactor, chore, test, style, perf, build, ci, revert

Exemples:
  git commit-fix "corrige NPE sur upload"
  git cm feat "ajoute pagination sur /users"
  git cm refactor "isole service UserSync"
EOF
}
...
set_alias() {
  ...
  if [ "$FORCE" = "1" ]; then
    ...
    if git config $SCOPE --get "alias.${name}" >/dev/null 2>&1; then
      echo "• alias.${name} existe déjà (skip). Utilise --force pour écraser."
    else
      git config $SCOPE "alias.${name}" "${value}"
      echo "✔ alias.${name}"
    fi
...
# Affiche ce qui a été ajouté
echo
echo "Terminé. Exemples:"
echo '  git commit-fix  "corrige bug X"'
echo '  git commit-feat "ajoute Y"'
echo '  git cm refactor "extrait logique Z"'
echo '  git amend   # pour amender sans changer le message'
